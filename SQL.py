        ...
        match table:
            case 'funcionario':
                try:
                    ...
                    self.cursor.execute("INSERT INTO endereco (pessoa_id, pais, cep, estado, cidade, bairro, rua, numero, complemento) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);", (last_id, kwargs.get('pais'), kwargs.get('cep'), kwargs.get('estado'), kwargs.get('cidade'), kwargs.get('bairro'), kwargs.get('logradouro'), kwargs.get('numero'), kwargs.get('complemento')))
                    self.conn.commit()

                    self.cursor.execute("INSERT INTO funcionario (funcionario_id, matricula, cargo, salario) VALUES (?, ?, ?, ?);", (last_id, kwargs.get('matricula'), kwargs.get('cargo'), kwargs.get('salario')))
                    self.conn.commit()
